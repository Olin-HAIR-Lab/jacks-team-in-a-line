...
agent_pos = {0: (0, 0), 1: (0, 3)}

# agent_number: [start_pos, pick_pos, drop_pos]
agents = {0: [(0, 0), (2, 2), (4, 0)], 1: [(0, 3), (2, 3), (4, 3)]}
...
for agent_k, agent_v in agents.items():
    ...
    for loc_i in range(len(agent_v) - 1):
        agent_paths[agent_k] += astar(agent_v[loc_i], agent_v[loc_i + 1], map_grid)


agent_next_pos = {}
step = 0
while not all([agent_pos[i] == agents[i][-1] for i in agents]) and step < 10:
    ...
    if len(set(all_hitboxes)) < len(all_hitboxes):
        ...
        drone_points = [
            point for point in list(agent_next_pos.values()) if point in duplicates
        ]
        ...
        for agent_a in bad_agents:
            for agent_b in bad_agents:
                if (
                    ...
                    and agent_a != agent_b
                    and (
                        agent_next_pos[agent_a] in get_hitbox(agent_next_pos[agent_b])
                        or agent_next_pos[agent_b]
                        in get_hitbox(agent_next_pos[agent_a])
                    )
                ):
