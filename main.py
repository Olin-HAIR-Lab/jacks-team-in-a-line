...
if __name__ == "__main__":
    ...
    # config_file = "./maps/twenty_task_warehouse_config.yaml"
    ...
    for i in range(len(pick_locations)):
        t = Task(
            pick_loc=Position(
                x=pick_locations[i][1][0],
                y=pick_locations[i][1][1],
    ...
    for i in range(num_agents):
        ...
        start = State(x_pos=agent_init[i][1][0], y_pos=agent_init[i][1][1])
    ...
    gcs = GroundControlSystem(agent_list=agent_list, task_list=task_list, env=env)
    ...
    sim = Simulation(
        ...
        fig1=fig1,
        fig2=fig2,
        fig_animated=fig_animated,
        fig3d_animated=fig3d_animated,
    )
